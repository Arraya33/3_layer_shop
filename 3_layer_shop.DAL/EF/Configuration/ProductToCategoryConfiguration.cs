...
namespace _3_layer_shop.DAL.EF.Configuration
{
    public class ProductToCategoryConfiguration : IEntityTypeConfiguration<ProductToCategory>
    {
        ...
        {
            builder.ToTable("ProductsToCategories").HasOne(ptc => ptc.Product).WithMany(product => product.ProductToCategories).HasForeignKey(ptc => ptc.ProductId);
            builder.HasOne(ptc => ptc.ProductCategory).WithMany(pc => pc.ProductToCategories).HasForeignKey(ptc => ptc.ProductCategoryId);
